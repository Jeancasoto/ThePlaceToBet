...
while contador_global <num :
    ...
    def jugadores_disponibles(num):
            ...
            for jug in lista_jugadores_disp:
                jug = jug.key
    ...
    def actualizar_clubes(num):
        ...
        for i in range(len(lista_alfa)):
            ...
            for j in range(len(lista_equipos)): 
                ...
                member_of = equipos["content"]["club"]
                #decision para poder agregar a la lista
                if member_of == club:
                    #agregar a la nueva lista el id del equipo
                    new_team_list.append(str(team_id))
                else:
                    print (str(team_id)+'No es miembro del club'+ str(club))
        if len(new_team_list) is not 0:
            clubes["content"]["equipos"] = str(new_team_list) 
            db.save(club)
        else:
...
agregar_a_couch = push_to_database(num)
